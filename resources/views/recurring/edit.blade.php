...
@section('title', 'Edit Recurring Bill - Flux')
...
<div class="form-header">
    <div>
        <h1>Edit Recurring Bill</h1>
        <p>Update your automatic recurring income or expense</p>
    </div>
    <div>
...
<div class="form-card">
    <form action="{{ route('recurring.update', $recurringBill->id) }}" method="POST" id="recurringForm">
        ...
        <div class="info-box warning">
            ...
            <div>
                <strong>Note:</strong>
                <p>Changes only affect future transactions. Past transactions remain unchanged.</p>
            </div>
        </div>
        ...
        <div class="form-row">
            <div class="form-group">
                ...
                <select id="type" name="type" class="form-select @error('type') error @enderror" required onchange="updateCategoryOptions()" disabled>
                    ...
                    <option value="expense" {{ old('type', $recurringBill->type) == 'expense' ? 'selected' : '' }}>Expense</option>
                    <option value="income" {{ old('type', $recurringBill->type) == 'income' ? 'selected' : '' }}>Income</option>
                </select>
                @error('type')
                    <div class="error-message">{{ $message }}</div>
                @enderror
            ...
            <div class="form-group">
                ...
                <select id="frequency" name="frequency" class="form-select @error('frequency') error @enderror" required>
                    ...
                    <option value="weekly" {{ old('frequency', $recurringBill->frequency) == 'weekly' ? 'selected' : '' }}>Weekly</option>
        ...
        <div class="form-group">
            <label class="form-label" for="amount">
                ...
                @if($currentCurrency == 'USD')
                    <small class="text-muted">Will be converted to IDR for storage</small>
                @endif
            </label>
            ...
            <p style="margin-top: 0.5rem; font-size: 0.75rem; color: var(--text-secondary-light);">
                @if($currentCurrency == 'IDR')
                    Stored as: Rp {{ number_format($recurringBill->amount, 0, ',', '.') }}
                @else
                    Stored as: Rp {{ number_format($recurringBill->amount, 0, ',', '.') }}
                @endif
            </p>
        ...
        <div class="form-group">
            ...
            <input type="text" 
                   ...
                   disabled>
            <p style="margin-top: 0.5rem; font-size: 0.75rem; color: var(--text-secondary-light);">
                Next payment will be processed automatically on this date
            </p>
        </div>
